...
const Education = forwardRef(({ t, tr, data }, ref) => {
  return (
    <section id="education" ref={ref} style={{ minHeight: '60vh', padding: '100px 64px', display: 'flex', alignItems: 'center', position: 'relative' }}>
      <div style={{ width: '100%', maxWidth: '900px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
          <div style={{ width: '32px', height: '1px', background: t.accent }} />
          <span style={{ fontSize: '10px', letterSpacing: '4px', textTransform: 'uppercase', color: t.accent }}>{tr.edu_subtitle}</span>
        </div>

        <h2 style={{ fontSize: '48px', fontWeight: '700', letterSpacing: '-2px', marginBottom: '48px' }}>
          {tr.edu_title_1} <span style={{ color: t.accent }}>{tr.edu_title_2}</span>
        </h2>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }} className="edu-grid">
          {data.educations.length === 0 ? (
            <p style={{ color: t.textFaint, fontSize: '13px' }}>{tr.edu_empty}</p>
          ) : data.educations.map(edu => (
            <div key={edu.id} style={{
              background: t.bg2, border: `0.5px solid ${t.border}`,
              borderLeft: `3px solid ${edu.accent === 'pink' ? t.pink : t.accent}`,
              borderRadius: '12px', padding: '24px', transition: 'transform 0.2s',
              ...
              <div style={{ fontSize: '10px', color: t.textFaint, letterSpacing: '1px' }}>
                {edu.start_year} — {edu.end_year || tr.edu_present}
              </div>
            </div>
          ))}
        </div>
